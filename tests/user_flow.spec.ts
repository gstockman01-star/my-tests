...
test('Полный цикл: Регистрация и Профиль (Стабильная версия)', async ({ page }) => {
  ...
  // Генерируем дату на день вперед для валидации
  ...
  // Используем ID, чтобы избежать ошибки дублирования, и динамическую дату
  await page.locator('#date-input-create').fill(taskDate);
  ...
  // Выбор пола через listbox/option
  await page.getByRole('button', { name: 'Пол' }).click();
  // Кликаем на first option в listbox (это должен быть "Мужской")
  await page.getByRole('option').first().click();

  // Сохранение с ожиданием загрузки страницы
  await page.getByRole('button', { name: 'Сохранить' }).click();
  // Ждём, пока сеть станет стабильной
  await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {
    // Игнорируем таймаут, так как сервер может быть медленным
  });
